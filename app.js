...
myApp.inventoryElement //querying ul list that will contain movie cards
  .addEventListener("click", function (event) {
    const linkToProvider =
      event.target.offsetParent.offsetParent.nextElementSibling.firstChild.href;
    const movieTitle =
      event.target.offsetParent.previousElementSibling.firstElementChild.title;
...
myApp.checkForDuplicates = function () {
  ...
  for (let i = 0; i < buttonArray.length; i++) {
    ...
    for (let i = 0; i < localItems.length; i++) {
      if (
        currentButton.offsetParent.firstElementChild.firstElementChild.title ===
        localItems[i].movieTitle
      ) {
